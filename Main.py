...
order_path = "haikou/data/order.csv"
...
env_args = {
    "driver_num" : 20,                    # 车辆数
    "order_path" : 'haikou/data/order.csv',     # 订单文件路径
    "GUI": True,
    "sumo_path" : 'C:/Program Files (x86)/Eclipse/Sumo/bin',  # 图形界面
    "sumocfg_path" : 'haikou/network/haikou.sumocfg',
    "time_interval":10,
...
for key in obs[0].keys():
    vehicle_dic[key] = obs[0][key]
for key in obs[1].keys():
    vehicle_dic[key] = obs[1][key]
for key in obs[2].keys():
    vehicle_dic[key] = obs[2][key]
action_generator = sm.Simulation(Config.Config(),vehicle_dic)
...
while not terminate:
    ...
    action = action_generator.step(obs)
